...
if (isset($_POST["submit"])) {
    ...
    $query = mysqli_query($mysqli, "INSERT INTO kommentare (Nutzername, Nachricht) VALUES ('$nutzername', '$nachricht')");
...
<body>
    ...
    <div class="about container animeX">
        ...
        <?php 
        if (!isset($_GET['id'])) {
            echo "Kein Spiel gewählt.";
            exit;
        }
     
        $id = (int) $_GET['id'];
     
        // JOIN mit der Kategorien-Tabelle
        $stmt = $mysqli->prepare("
            SELECT s.Titel, s.Beschreibung, s.Preis, k.Name AS Kategorie, s.Veröffentlichungsdatum
            FROM spiele s
            LEFT JOIN kategorien k ON s.Kategorie_ID = k.Kategorie_ID
            WHERE s.Spiel_ID = ?
        ");
        ...
        if ($spiel = $result->fetch_assoc()) {
            echo "<h1>" . htmlspecialchars($spiel['Titel']) . "</h1>";
            echo "<p><strong>Beschreibung:</strong> " . nl2br(htmlspecialchars($spiel['Beschreibung'])) . "</p>";
            echo "<p><strong>Preis:</strong> " . htmlspecialchars($spiel['Preis']) . " €</p>";
            echo "<p><strong>Kategorie:</strong> " . htmlspecialchars($spiel['Kategorie']) . "</p>";
            echo "<p><strong>Veröffentlichung:</strong> " . htmlspecialchars($spiel['Veröffentlichungsdatum']) . "</p>";
        } else {
